...
new: matrix instance timer general formulations solution usercut_obj kernel_search heuristic_solution
	$(CC) $(CFLAGS) $(CFLAGS2) $(MATRIX_HPP) $(INSTANCE_SRC) $(TIMER_SRC) $(GENERAL_SRC) $(FORM_SRC) $(SOLUTION_HPP) $(USERCUT_SRC) $(KERNEL_SEARCH_SRC) $(HEURISTIC_SOLUTION_SRC) $(MAIN_SRC) -o $(PROG_BIN)/stop $(CLNFLAGS)
