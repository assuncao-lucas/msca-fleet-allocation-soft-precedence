...
UserCut::UserCut(int lhs_dimension, double abs_violation, int type)
{
    this->norm_ = 0.0;
...
void UserCut::UpdateMeasures()
{
    ...
    this->norm_ = sqrt(1.0 * ((int)((this->lhs_nonzero_coefficients_indexes_).size())));
    this->density_ = (1.0 * ((int)((this->lhs_nonzero_coefficients_indexes_).size()))) / (1.0 * (this->lhs_dimension_));
...
double UserCut::operator*(UserCut &other)
{
    ...
    if (this->lhs_dimension_ != other.lhs_dimension_)
    {
        throw 2;
    }
    else
    {
        ...
        cuts_inner_product = 1.0 * (((this->lhs_) & (other.lhs_)).count());
    }
